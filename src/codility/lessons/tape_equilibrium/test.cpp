...
void
test_trivial_cases() {
  const std::vector<std::pair<std::vector<int>,int>> test_cases({
    std::make_pair(std::vector<int>({3,1,2,4,3}), 1),
    std::make_pair(std::vector<int>({2,1}), 1),
    std::make_pair(std::vector<int>({1,2,3,-1,-2,-3}), 2),
  });
  /// empty vector
  for (const auto test_case: test_cases) {
    const auto result = ma::tape_equilibrium(test_case.first);
